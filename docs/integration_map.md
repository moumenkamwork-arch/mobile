...
| **حالة الربط الحالية (موبايل فقط)**                   | **✅ ~50 موصولة حياً + Chat Realtime** | ...السابقة + `GET/POST /chats` · `GET/POST /chats/:id/messages` · `PATCH /chats/:id/read` · Supabase Realtime · **notifications (5)** · **`POST /upload/image` + `POST /profiles/me/avatar`** · **`POST /stories` + `DELETE /stories/:id`** · **`POST/PUT/DELETE /offers|/services` + `GET /offers|/services/profile/:id`** (Ads أُزيلت من الموبايل 2026-07-23، شوف قسم 3.11) · **`POST /saved` · `GET /follows/followers/:id`** · **`POST/DELETE /blocks/:id` · `GET /blocks(/:id/status)`** · **`POST /reports`** · **`DELETE /profiles/me`**. الباقي بموبايل v1 (cover فقط) لسا مو مربوط؛ الباقي إجمالاً كله admin/v2 + `ads` (باقية بالباك بلا مستهلك بالموبايل). |
...
| 9   | Stories             | 5       | 5          | ✅ **الكل v1 (2026-07-21)**: GET عبر /home (مجمّعة حسب الشخص)، POST (رفع + إنشاء)، DELETE (لصاحبها فقط، من قائمة الـ ⋮ بالمشاهد).                                                                            |
...
| -------------------- | -------------------------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Stories              | `stories[]`                            | 100%  | ✅ **موصول بالكامل (2026-07-21)** — `HomeStoryDto.groupedFromJson` يجمّع الصفوف المسطّحة من الباك حسب صاحبها (دائرة وحدة/شخص، مش دائرة لكل قصة). مُتحقَّق حياً بين جهازين حقيقيين (المالك + صديق). |
...
### 3.11 — إدارة المحتوى (نشر + تعديل + حذف + "منشوراتي") — ✅ موصولة، **بعد إزالة Ads (2026-07-23)**

> ⚠️ **تحديث جوهري 2026-07-23:** تبيّن إنو "Ads" ما كانت أصلاً جزء من التصميم يلي أعطاه الزبون (Prototype) — التطبيق المطلوب فيه بس **Offers** و**Services**. أُزيلت كل آثار الإعلان من كود الموبايل بالكامل (شاشة، route، repository، الظهور بالـ Home/Search/My Listings/Reports/الحظر) — **بلا أي تعديل أو حذف بالباك** (جدول `ads` وكل مساراتو باقين تماماً، تحسباً لـ v2). `influencer` (يلي كان دوره الوحيد بالنشر مرتبط بـ Ads) صار يستخدم **Add Offer** بدلاً منه — الباك عدّل `requireAccountType` على `POST /offers` ليشمل `influencer`. الجدول تحت يعكس الوضع الحالي (Offers/Services فقط)؛ صفوف `ads` تُركت **كمرجع تاريخي مشطوب** لتوثيق شو انبنى وشو انحذف.

| Endpoint                    | Method | Backend | Role                                    | ملاحظة                                                                                                                          |
| --------------------------- | ------ | ------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `POST /offers`              | POST   | ✅201   | company, service_provider, **influencer** | ✅ **موصول** — `OffersRepository.createOffer` + category picker حقيقي + `PromooImageUploadField`. صلاحية `influencer` أُضيفت 2026-07-23 (بديل Ads). |
...
| ~~`POST/PUT/DELETE /ads`~~  | —      | ✅ (باقي بالباك) | —                                | ⛔ **مو مستهلك من الموبايل بعد 2026-07-23** — كانت موصولة (Phase 12، 2026-07-22) عبر `AddAdWizardScreen`/`AdsRepository`، انحذفت كل ملفاتها من الموبايل. الباك ما تغيّر (`ad.service.ts`/`ad.routes.ts` كاملين، وفيهم `deleteAd` المُضاف 2026-07-22). |
| ~~`GET /ads/profile/:id`~~  | —      | ✅ (باقي بالباك) | —                                | ⛔ نفس الشي — كان يغذّي قسم "الإعلانات" بـ My Listings، انحذف القسم كامل مع الميزة.                                              |

> **شاشة "منشوراتي" (`MyListingsScreen`/`MyListingsController`)** تحت قائمة البروفايل (تظهر لمن يقدر ينشر — `canCreateAnything`): تجيب العروض والخدمات بالتوازي، تعرضهم بأقسام مع حالة كل عنصر، التعديل يفتح شاشة الإضافة المناسبة مع `editing:` (prefill + PUT)، والحذف بتأكيد. الشاشتان تتشاركان chrome عبر `add_form_widgets.dart` وتُظهرهما `accountCapabilitiesProvider` حسب `account_type`. التفاصيل الكاملة بـ [MEMORY_BANK.md](MEMORY_BANK.md) §5.
...
| ✅ إدارة المحتوى (نشر/تعديل/حذف/منشوراتي) | 8 | **90%**  | **موصول بالكامل — Offers/Services فقط** (Phase 12 نشر + 2026-07-22 تعديل/حذف/منشوراتي؛ Ads أُزيلت من الموبايل 2026-07-23، شوف قسم 3.11) — `POST/PUT/DELETE` + `GET /*/profile/:id` |
...
| **الإجمالي المرجّح**     | **~58**   | **≈90%** |                                                                                           |

---

## 5. تعارض Add Offer vs Add Ad — ✅ اتحل نهائياً بحذف Ad (2026-07-23)

<div dir="rtl">

**الخلفية التاريخية:** التعارض الأصلي (صف واحد "Add New Offer" يفتح wizard الإعلان → أسماء/صلاحيات متضاربة → 403 محتمل) **حُلّ أول مرة** بفصل 3 شاشات منفصلة (`add_offer_screen.dart` · `add_service_screen.dart` · Ad wizard)، و`accountCapabilitiesProvider` ([account_capabilities.dart](../lib/features/profile/presentation/controllers/account_capabilities.dart)) كان يقرأ `account_type` ويُظهر الإنشاءات المسموحة فقط (Offer=company/service_provider · Ad=company/influencer · Service=company/service_provider · guest/user=لا شيء)، وبتاريخ 2026-07-22 صارت الثلاثة موصولة حياً بـ `POST /ads|/offers|/services` (Phase 12).

**التطور النهائي (2026-07-23):** تبيّن إنو أصل التعارض كان **التصميم الأصلي (Prototype) ما فيه Ads إطلاقاً** — بس Offers و Services. القرار: **حذف Ad نهائياً من الموبايل** (لا شاشة، لا route، لا repository) مع إبقاء الباك بلا أي تغيير. `influencer` (يلي كانت صلاحيته الوحيدة Ads) صار عندو `canAddOffer` بدلاً منها — الباك أضاف `influencer` لـ `requireAccountType` على `POST /offers`. التعارض الآن **معدوم بنيوياً**: صفحتان بس (Offer/Service)، بلا أي احتمال تضارب تسمية/صلاحية. التفاصيل الكاملة: [MEMORY_BANK.md](MEMORY_BANK.md) §5.
...
| 📸 Stories create/delete              | ~~`POST /stories`, `DELETE /stories/:id` (2)~~        | ✅ موصولة (2026-07-21) — إنشاء + حذف حي، مش display-only.                                                                                                                          |
