...
10. ~~Notifications~~ ✅ **تمّت (Phase 10 — 2026-07-19)** — `GET /notifications` + `PATCH /notifications/:id/read` + `PATCH /notifications/read-all` + `DELETE /notifications/:id` + `POST /notifications/token` موصولة. (صلّحت مشكلة الـ `chat-room-1` 400: الإشعارات صارت حقيقية فتحمل room id حقيقي يفتح المحادثة صح.) دفع FCM لسا مؤجل.
11. **أزرار مدعومة بالباك بلا توصيل (بقية الأقسام):** نشر الإعلان/العرض/الخدمة، رفع الأفاتار/الصور (Upload)، زر حفظ من الكروت، قوائم المتابِعين/المتابَعين.
...
| endpoints مؤجلة v2 (Stripe/Reports/Featured/FCM push) | **~22**                                | القسم 6 — Notifications REST صارت موصولة (Phase 10)، بقي دفع FCM فقط                                                                                                                                                                                                                                                                                                    |
...
| **حالة الربط الحالية**                                | **✅ ~30 موصولة حياً + Chat Realtime** | ...السابقة + `GET/POST /chats` · `GET/POST /chats/:id/messages` · `PATCH /chats/:id/read` · اشتراك Supabase Realtime على `messages` · **`GET /notifications` · `PATCH /notifications/:id/read` · `PATCH /notifications/read-all` · `DELETE /notifications/:id` · `POST /notifications/token`**. الباقي (~20) لسا fake (أبرزه: Upload، نشر المحتوى، زر الحفظ من الكروت). |
...
| 14  | Notifications       | 5       | 5          | ✅ موصولة (Phase 10): list/read/read-all/delete/token. دفع FCM لسا مؤجل.                                              |
...
| `POST /notifications/token`     | POST   | ✅      | موصول (تسجيل توكن جهاز). دفع FCM الفعلي لسا مؤجل.                                                                                                 |
...
| 🔔 Notifications                      | ~~`/notifications*` (5)~~                             | ✅ موصولة (Phase 10). دفع FCM فقط لسا مؤجل.                                                                                                                                        |
